...
const rejectSchema = z.object({
  ...
  reason: z.string().min(1, 'Reason is required for rejecting media'),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to reject media', details: error.message },
        { status: 500 }
      );
    }
    ...
    console.error('Error in media reject endpoint:', error);
    return NextResponse.json(
