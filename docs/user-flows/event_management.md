...
Event organizers can change the status of an event using the status selector component on the event details page. This component provides a dropdown interface with clear descriptions of each status option.

### Status Change Effects
When an event's status changes:

- **Draft to Published**: Makes the event visible to invited attendees, enables QR code scanning
- **Published to Completed**: Archives the event, disables new uploads, but keeps gallery accessible
- **Any Status to Cancelled**: Disables all event functionality except viewing for organizers
- **Cancelled to Draft/Published**: Re-enables event functionality based on the selected status
...
- **Share**: Send the QR code directly via email or messaging apps
...
This approach balances flexibility for organizers with reliability for attendees, ensuring that printed materials and distributed QR codes remain functional throughout the event lifecycle.
...
2. An invitation email is automatically sent (if enabled)
